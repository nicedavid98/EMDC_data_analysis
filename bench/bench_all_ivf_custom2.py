...
def main():
    ...
    if "search" in args.todo:
        ...
        # Node0 을 가득 채우기 위한 처리.
        gb_size = 6
        logging.info(f"Searching : allocating {gb_size} dummy memory")
        allocate_memory(gb_size)
        time.sleep(5)
        logging.info(f"Searching : allocated {gb_size} dummy memory")
        ...
        # 검색 시작 전 Node0 에 할당된 Dummy Memory 해제.
        logging.info(f"Searching : deallocating {gb_size} dummy memory")
        release_memory()
        time.sleep(5)
        logging.info(f"Searching : deallocated {gb_size} dummy memory")
