...
def main():
    ...
    if "search" in args.todo:
        ...
        # Node0 비활성화 : 인덱스 파일이 CXL Node로 할당되기 위함.
        disable_cores_on_node(0)

        logging.info("Starting searching : Reading Index File")
        print("================== Searching")
        if not index:
            if args.indexfile:
        ...
        # Node1을 CXL Node와 같이 CPU 독립적으로 설정.
        disable_cores_on_node(1)

        logging.info(f"Starting Searching : Run Search.")
        start_time = time.time()
        try:
            ...
            logger_thread.join()
            return
        logging.info(f"Searching completed in {time.time() - start_time:.2f} seconds")
